...
CreatureCellOrParticleDescription EditorModel::getInspectedEntity(uint64_t id) const
{
    return _inspectedEntityById.at(id);
}

void EditorModel::addInspectedEntity(CreatureCellOrParticleDescription const& entity)
{
    _inspectedEntityById.emplace(DescriptionEditService::get().getId(entity), entity);
}

void EditorModel::setInspectedEntities(std::vector<CreatureCellOrParticleDescription> const& inspectedEntities)
{
