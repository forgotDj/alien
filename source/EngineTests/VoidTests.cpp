...
TEST_F(VoidTests, destroyWhenReady_transferEnergyToNeighbors)
{
    auto data = Desc()
                    .addCreature({
                        ObjectDesc().id(1).pos({10.0f, 10.0f}).fixed(true).type(CellDesc().usableEnergy(100.0f).cellType(BaseDesc())),
                        ObjectDesc().id(2).pos({11.0f, 10.0f}).fixed(true).type(CellDesc().usableEnergy(300.0f).cellType(VoidDesc())),
                        ObjectDesc().id(3).pos({12.0f, 10.0f}).fixed(true).type(CellDesc().usableEnergy(100.0f).cellType(BaseDesc())),
                    })
                    .addConnection(1, 2)
                    .addConnection(2, 3);
